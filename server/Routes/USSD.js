    ...
    .post('/', async (req,res) => {
        const { phoneNumber, text } = req.body
            ...
            if ( text !== '' ){
                inputs = text.split('*')
                    if (inputs.length > 0){
                        if (parseInt(inputs[0]) === 1) await displayOptionOne(phoneNumber)
...
const
    ...
    displayOptionOne = async Phonenumber =>{
        response = `CON 1. View Medical History \n 2. View/Edit Personal Information`
            if (inputs.length > 1){
                if (parseInt(inputs[1]) === 1){
                    response = `CON Enter Your PIN.`
                    if (inputs.length > 2) await displayMedicalHistory(Phonenumber)
                } else if (parseInt(inputs[1]) === 2){
                    response = `CON Enter Your Pin`
                    if (inputs.length > 2) await displayUserData(Phonenumber)
                }
            }
    },
